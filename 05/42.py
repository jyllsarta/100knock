...
if __name__ == "__main__":
    ...
    for i in range(len(s)):
        print("original : {}".format(s[i].toString()))

        #かかりもとの一覧をタブ区切りで表示
        print("    from : {}".format("\t".join([s[idx].toString(depressPunctuation=True) for idx in s[i].srcs])))
        print("    to   : {}".format(s[s[i].dst].toString(depressPunctuation=True)))
        print()
